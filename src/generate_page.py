...
def generate_page_recursive(dir_path_content,template_path,dest_path,basepath):
    print(f"Generating pages (recursively) \nfrom: {dir_path_content} \nto : {dest_path}.")
    list_of_dir = os.listdir(dir_path_content)
    for item in list_of_dir:
        if item.endswith(".md"):
            generate_page(os.path.join(dir_path_content,item),template_path, os.path.join(dest_path,item.replace(".md",".html")),basepath)
        if os.path.isdir(os.path.join(dir_path_content,item)):
            os.mkdir(os.path.join(dest_path,item))
            generate_page_recursive(os.path.join(dir_path_content,item),template_path,os.path.join(dest_path,item),basepath)